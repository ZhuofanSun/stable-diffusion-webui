"""
更改后的/Users/sunzhuofan/sdai/my_sd_webui/extensions/sd-webui-controlnet/annotator/leres/__init__.py
解决ControlNet - Depth - leres与处理器
【Bug】RuntimeError: Input type (MPSFloatType) and weight type (torch.FloatTensor) should be the sam
"""
import cv2
import numpy as np
import torch
...
def apply_leres(input_image, thr_a, thr_b, boost=False):
    ...
    if boost and pix2pixmodel is None:
        ...
        if not torch.cuda.is_available():
            #################################
            device = torch.device('mps')  # 定义 device 变量
            #################################
